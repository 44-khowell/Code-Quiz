...
// Get player Correct scores from the Quiz 
...
// Get player Wrong scores from the Quiz 
function renderWrongScores() {

  scoreCountWrong = localStorage.getItem('scoreCountWrong');    // Used to get value of Scorecount 

  // Check if fetched storage variable is empty before displaying to page 
  if (scoreCountWrong === null) {
    console.log('No value stored in localstorage');
  return;
  }  
  return scoreCountWrong;
}
...
function loseGuess(action) {
  ...
  if (secondsLeft > 0 && (questionNumber <= 3)) {
    ...
    // Get wrong score count 
    let wrongScoreCount = renderWrongScores();
    console.log('Debug: wrong score count is: ', wrongScoreCount);
    if (wrongScoreCount == 5) {
      // Set the overall score count in local storage to zero
      scoreCountCorrect = 0;
      localStorage.setItem('scoreCountCorrect',scoreCountCorrect);
      clearInterval(timerInterval);

      //getFinalScore();
      //console.log('Debug: Getting score before clearing it');
      clearScreen();
    }    
  }
